...
Migrates existing hardcoded data to database KBs.
This is Phase 1 of data import - quick start with existing data.

Run from backend directory:
    python -m app.platform.data.scripts.import_kb_from_hardcoded
"""
import sys
from pathlib import Path

# Add backend directory to Python path
# The script should be run from backend directory
backend_dir = Path(__file__).parent.parent.parent.parent
if str(backend_dir) not in sys.path:
    sys.path.insert(0, str(backend_dir))

# Import after path is set (avoid platform module conflict)
...
from app.platform.engines.diagnosis_engine.kb_thresholds import MEDICAL_THRESHOLDS, NUTRITION_THRESHOLDS
from app.platform.engines.mnt_engine.kb_mnt_rules import MNT_RULES
import logging
